...
#define model_fn "model/op2-model-neural-network.dat"
// Mẫu test
...
{
    __shared__ double A_tile[TILE_BLOCK][TILE_BLOCK];
    __shared__ double B_tile[TILE_BLOCK][TILE_BLOCK];
    ...
    int Row = by * TILE_BLOCK + ty;
    int Col = bx * TILE_BLOCK + tx;
    if (Row < m && Col < k)
    {
        double pvalue = 0;
        for (int t = 0; t < (n - 1) / TILE_BLOCK + 1; ++t)
        {
            if (Row < m && t * TILE_BLOCK + tx < n)
                A_tile[ty][tx] = A[Row * n + t * TILE_BLOCK + tx];
            else
                A_tile[ty][tx] = 0;

            if (t * TILE_BLOCK + ty < n && Col < k)
                B_tile[ty][tx] = B[(t * TILE_BLOCK + ty) * k + Col];
            else
                B_tile[ty][tx] = 0;
...
void forward_pass_and_back_propagation(double *weight_A, double *weight_B, double *weight_C, int *labels, int training_example, double **images, double **layer_inputs, double **layer_outputs)
{
    ...
    mCols = nRows = 1;
    kCols = 10;
    dim3 blockSize_calculate_for_update_weight_C(16, 16);
    ...
    update_weight_kernel_shared<<<gridSize_calculate_for_update_weight_C, blockSize_calculate_for_update_weight_C>>>(mRows, mCols, kCols, d_layer_outputs_2_transpose, d_local_gradient_3, d_weight_C);
    cudaDeviceSynchronize(); // Synchronize to make sure that the kernel is finished
    ...
    dim3 blockSize_calculate_for_update_weight_B(16, 16);
    ...
    update_weight_kernel_shared<<<gridSize_calculate_for_update_weight_B, blockSize_calculate_for_update_weight_B>>>(mRows, mCols, kCols, d_layer_outputs_1_transpose, d_local_gradient_2, d_weight_B);
    cudaDeviceSynchronize(); // Synchronize to make sure that the kernel is finished
    ...
    dim3 blockSize_calculate_for_update_weight_A(16, 16);
    ...
    update_weight_kernel_shared<<<gridSize_calculate_for_update_weight_A, blockSize_calculate_for_update_weight_A>>>(mRows, mCols, kCols, d_layer_outputs_0_transpose, d_local_gradient_1, d_weight_A);
    cudaDeviceSynchronize(); // Synchronize to make sure that the kernel is finished
